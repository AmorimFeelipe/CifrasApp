...
export default function ChordViewer() {
  ...
  useEffect(() => {
    const loadSongIndex = async () => {
      const modules = import.meta.glob('../Chords/*.json', { query: '?raw', import: 'default' });

      const list: SongMeta[] = Object.entries(modules).map(([path, loader]) => {
        const filename = path.split('/').pop()?.replace('.json', '') || 'Desconhecido';
  ...
  const loadSong = async (meta: SongMeta) => {
    ...
    try {
      ...
      const songData: ChordFile = {
        title: jsonData.title || meta.title,
        artist: jsonData.artist || meta.artist,
        key: jsonData.key,
      ...
      alert("Não foi possível carregar ou processar este arquivo JSON.");
    } finally {
      setIsLoading(false);
    }
  };
